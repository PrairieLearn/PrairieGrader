...
module.exports.inService = async () => {
    ...
    await util.promisify(autoscaling.completeLifecycleAction)(params);
    logger.info(`lifecycle action ${params.LifecycleActionResult} sent to hook ${params.LifecycleActionResult}`);
};
...
module.exports.abandonLaunch = async () => {
    ...
    if (lifecycleState == 'Wait') {
        ...
        await util.promisify(autoscaling.completeLifecycleAction)(params);
        logger.info(`lifecycle.abandonLaunch(): action ${params.LifecycleActionResult} sent to hook ${params.LifecycleActionResult}`);
    } else {
...
function heartbeat() {
    if (lifecycleState == 'Wait') {
        ...
        autoscaling.recordLifecycleActionHeartbeat(params, (err, data) => {
