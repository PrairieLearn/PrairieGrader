...
config.loadConfig = function(callback) {
    ...
    async.series([
        ...
        (callback) => {
            exportedConfig.awsServiceGlobalOptions = {};
            if (exportedConfig.isDevelopment && exportedConfig.awsEndpoint) {
                exportedConfig.awsServiceGlobalOptions.endpoint = exportedConfig.awsEndpoint;
                logger.info('Setting AWS endpoint to ' + exportedConfig.awsEndpoint);
            }

            MetadataService = new AWS.MetadataService(exportedConfig.awsServiceGlobalOptions);
        ...
        (callback) => {
            ...
            const ec2 = new AWS.EC2(exportedConfig.awsServiceGlobalOptions);
            ...
            ec2.describeTags(params, function(err, data) {
                ...
                const secretsManager = new AWS.SecretsManager(exportedConfig.awsServiceGlobalOptions);
        ...
        (callback) => {
            ...
            var autoscaling = new AWS.AutoScaling(exportedConfig.awsServiceGlobalOptions);
...
function getQueueUrl(prefix, callback) {
    ...
    if (exportedConfig[queueUrlKey]) {
        ...
        const sqs = new AWS.SQS(exportedConfig.awsServiceGlobalOptions);
        const params = {
