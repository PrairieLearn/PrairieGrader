...
function initSqs(callback) {
  ...
  if (config.queueUrl) {
    logger.verbose(`Using queue url from config: ${config.queueUrl}`);
    this.queueUrl = config.queueUrl;
    callback(null);
  } else if (config.queueName) {
    logger.verbose(`Loading url for queue "${config.queueName}"`);
    const params = {
    ...
    this.sqs.getQueueUrl(params, (err, data) => {
        ...
        logger.verbose(`Loaded url for queue "${config.queueName}": ${config.queueUrl}`);
        callback(null);
    });
  }
}
...
async function initRabbitMq() {
  ...
  return new Promise((resolve) => {
    async function tryConnect() {
      try {
        that.conn = await amqp.connect(config.rabbitMqHost);
        resolve();
...
module.exports.init = function(callback) {
  logger.verbose(`Initializing with queue type: ${config.queueType}`);
