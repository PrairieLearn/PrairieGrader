...
var FORCED_REGISTRY = '078374015580.dkr.ecr.us-east-2.amazonaws.com';
...
module.exports = function(callback) {
    ...
    async.waterfall([
        ...
        (callback) => {
            ...
            sqldb.query(sql.select_all_images, [], (err, results) => {
                if (ERR(err, callback)) return;
        ...
        (images, callback) => {
            ...
            async.each(images, (image, callback) => {
                logger.info(`Pulling latest version of "${image}" image from ${FORCED_REGISTRY}`);
                var repository = new dockerUtil.DockerName(image);
                repository.registry = FORCED_REGISTRY;
                const params = {
