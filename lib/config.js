...
const exportedConfig = module.exports.config = {};
    ...
    async.series([
        (callback) => {
            configLib.loadConfigForEnvironment(configDir, env, (err, loadedConfig) => {
                ...
                // We need to pass the level from this config to the logger
                logger.setLevel(exportedConfig.globalLoggerLevel);
                callback(null);
            });
        },
        (callback) => {
            ...
            fs.readFile('./aws-config.json', (err, awsConfig) => {
                if (err) {
                    logger.verbose('Missing aws-config.json; credentials should be supplied by EC2 Metadata Service');
                    AWS.config.update({'region': 'us-east-2'});
                } else {
                    logger.verbose('Loading AWS config from aws-config.json');
        ...
        (callback) => {
            ...
            if (exportedConfig.useCloudWatchLogging) {
                ...
                logger.verbose(`CloudWatch logging enabled; logging to ${groupName}/${streamName}`);
            }
            callback(null);
        },
        (callback) => {
            ...
            if (exportedConfig.queueUrl) {
                logger.verbose(`Using queue url from config: ${exportedConfig.queueUrl}`);
                callback(null);
            } else {
                logger.verbose(`Loading url for queue "${exportedConfig.queueName}"`);
                ...
                sqs.getQueueUrl(params, (err, data) => {
                    ...
                    logger.verbose(`Loaded url for queue "${exportedConfig.queueName}": ${exportedConfig.queueUrl}`);
                    callback(null);
                });
            }
        }
