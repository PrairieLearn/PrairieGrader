...
module.exports = function(queue, receiveCallback, doneCallback) {
    let message, messageContent, jobCanceled;
    async.series([
        (callback) => {
        ...
        (callback) => {
            const valid = messageSchema(messageContent);
            if (!valid) {
        ...
        (callback) => {
            const timeout = messageContent.timeout || config.defaultTimeout;
            ...
            queue.updateMessageTtl(message, newTimeout, (err) => {
                if (ERR(err, callback)) return;
                return callback(null);
            });
        },
        (callback) => {
            ...
            const params = {
                grading_job_id: messageContent.jobId,
            };
        ...
        (callback) => {
            ...
            if (jobCanceled) {
                globalLogger.info(`Job ${messageContent.jobId} was canceled; skipping job.`);
                return callback(null);
            }

            receiveCallback(messageContent, (err) => {
                globalLogger.info(`Job ${messageContent.jobId} errored.`);
                callback(err);
            }, () => {
                globalLogger.info(`Job ${messageContent.jobId} finished successfully.`);
                callback(null);
            });
        },
        (callback) => {
